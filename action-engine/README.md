...
**급함 등급(7일 · 30일)은 법령이 아니라 팀 기준이다.** 법이 정한 것은 기한마다의 길이뿐이다. 근거는 `deadlines.json` 의 `severity` 에 적었고 `rules._severity` 가 거기서 읽는다.

| 등급 | 남은 날 | 근거 |
|---|---|---|
| `critical` | 7일 이하 | 가장 짧은 법정 불복 기한이 재정신청 10일(형사소송법 제260조 제3항). 서면 작성 · 제출과 주말을 빼면 7일이 당장 착수할 선 |
| `soon` | 30일 이하 | 항고 · 수사중지 이의제기 · 비공개 결정 이의신청이 모두 30일. 30일짜리 기한이 통지 받은 날부터 '곧'으로 보이게 |

말일이 토요일 · 공휴일이면 다음 날로 늘어나지만(제66조 제3항) 공소시효는 늘어나지 않으므로, 등급은 늘어나는 날을 믿지 않고 계산한 만료일로만 정한다.
