...
NOT_NAMES = {
    ...
    "소재불명", "소재불상", "행방불명",  # "참고인 소재불명으로" — 통지서 사유란이 이름 자리로 읽히던 것
    "물건", "상품", "택배", "금액", "연락", "전화", "문자", "사실", "내용", "경찰", "수사", "조사", "송금", "입금",
    "이체", "거래", "대화", "판매", "구매", "피해", "계좌로", "측은", "메모", "통장", "은행",
    "경위", "경사", "경장", "순경", "경감", "경정", "이름", "한테", "바뀌었다고",
}

ROLE_WORDS = (
    "피고소인|피고인|피의자|피해자|고소인|고발인|신고인|진정인|진술인|참고인|증인|원고|피고|판매자|구매자|"
    "예금주|수취인|담당\\s*수사관|담당\\s*형사|수사관|조사관|형사|경위|경사|경장|순경|경감|경정|검사|판사|재판장|"
    "실종자|목격자|제보자|아들|딸|남편|아내|어머니|아버지"
)
POLICE_RANKS = "경위|경사|경장|순경|경감|경정|총경"
# 같은 당사자를 절차 단계마다 다르게 부르는 역할 묶음 (coreference 근거)
ROLE_GROUPS = {
    "accused": {"피고소인", "피고인", "피의자", "피고", "판매자"},
    "victim": {"피해자", "고소인", "신고인", "진정인", "원고", "구매자"},
    "police": {"수사관", "담당수사관", "담당형사", "형사", "조사관", "경위", "경사", "경장", "순경", "경감", "경정"},
    "missing": {"실종자"},
}

PERSON_ROLE = re.compile(
    rf"(?P<role>{ROLE_WORDS}|성명|이름)\s*(?:[:：]\s*|\s+)(?:(?:{POLICE_RANKS})\s+)?"
    rf"(?P<name>[가-힣][가-힣{MASK_CLASS}]{{1,4}})"
)
# 보도 바이라인 '김○○ 기자'는 사건 인물이 아니다
PERSON_MASKED = re.compile(rf"(?<![가-힣])(?P<name>[가-힣][{MASK_CLASS}]{{1,2}})(?![가-힣{MASK_CLASS}])(?!\s*기자)")
